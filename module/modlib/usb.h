...
struct USBHostControllerDriver {
	...
	enum USBTransferStatus (*transfer_packet)(
		void *private, unsigned int addr, unsigned int endpoint, const struct USBPacket *packets,
		unsigned int num
	);
};

static inline void usb_register_host_controller(
	void *private, const char *name, unsigned int num_ports,
...
static inline enum USBTransferStatus usb_control_transfer_in(
	struct USBBus *bus, unsigned int addr, unsigned int endpoint, void *setup, void *payload,
	int size
) {
	return kernsrv->usb_control_transfer_in(bus, addr, endpoint, setup, payload, size);
}

static inline enum USBTransferStatus usb_control_transfer_nodata(
	struct USBBus *bus, unsigned int addr, unsigned int endpoint, void *setup
) {
	return usb_control_transfer_nodata(bus, addr, endpoint, setup);
}

static inline int usb_get_standard_descriptor(
...
static inline int usb_get_class_descriptor(
	struct USBDevice *dev, unsigned int desc_type, unsigned int desc_index, void *buffer,
	unsigned int size
) {
...
static inline int usb_get_configuration_descriptor(
...
static inline int usb_set_configuration(struct USBDevice *dev, uint8_t configuration_value) {
