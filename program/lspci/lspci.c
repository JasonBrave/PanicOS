	...
	for (int i = 0; i < 6; i++) {
		if (cfg->bar[i] && (cfg->bar[i] & 1)) {
			printf(
				"BAR%d IO %x - %lx\n",
				i,
				(unsigned int)cfg->bar[i] & 0xFFFFFFFC,
...
static void lspci_print_type1_header(const void *cfg_space, const struct PciKcallResource *pcires) {
	...
	printf(
		"Bridge Primary %x Secondary %x Subordinate %x\n", cfg->pribus, cfg->secbus, cfg->subbus
	);
	for (int i = 0; i < 2; i++) {
		if (cfg->bar[i] && (cfg->bar[i] & 1)) {
			printf(
	...
	printf(
		"Memory Window %x - %x\n", (cfg->mem_base & 0xfff0) << 16, (cfg->mem_limit & 0xfff0) << 16
	);
...
void lspci_print_device(
	unsigned int bus, unsigned int device, unsigned int function, const void *cfg_space,
	const char *drv_name, const struct PciKcallResource *pcires
) {
	...
	if (device_name) {
		printf(
			...
			cfg->vendor,
			cfg->device,
			pci_class_to_str(cfg->pclass, cfg->subclass, cfg->progif)
		);
	} else {
		printf(
			"%x:%x.%x Vendor %x Device %x [%s]\n",
			bus,
			device,
			function,
			cfg->vendor,
			...
			while (cap_off) {
				if (*((uint8_t *)cfg + cap_off) == 0x10) {
					...
					if (verbose_level > 2) {
						...
						printf(
							"    Message Control Enable%s, %d/%d Messages, "
						...
						if ((msgctl & 1) && (msgctl & (1 << 7))) {
							printf(
								"    Message address %lx, Message data %x\n",
					...
					if (verbose_level > 2) {
						uint16_t msgctl = *(uint16_t *)(cfg_space + cap_off + 2);
						printf(
							"    Message Control Enable%s, FuncMask%s, Table Size %d\n",
							(msgctl & (1 << 15)) ? "+" : "-",
							(msgctl & (1 << 14)) ? "+" : "-",
							(msgctl & 0x7ff) + 1
						);
						...
						printf(
							"    Table offset %lx BAR %ld\n", tableoff & 0xfffffff8, tableoff & 7
						);
...
int main(int argc, char *argv[]) {
	...
	while (pciid) {
		...
		pci_get_resource(pciid, &pcires);
		lspci_print_device(
			pciaddr.bus, pciaddr.device, pciaddr.function, cfg_space, drv_name, &pcires
		);
		pciid = pci_get_next_addr(pciid, &pciaddr);
	}
}
