...
void pata_adapter_dev_init(struct PCIDevice *pcidev) {
	...
	cprintf(
		"[ata] Adapter %d:%d.%d %s %s IDE Controller\n",
		addr->bus,
		addr->device,
		addr->function,
		bus_master_str[adapter->bus_master],
	...
	cprintf(
		"[ata] cmd0 0x%x ctl0 0x%x cmd1 0x%x ctl1 0x%x bmdma 0x%x\n",
		adapter->cmdblock_base[0],
		adapter->control_base[0],
		adapter->cmdblock_base[1],
		adapter->control_base[1],
...
void pata_adapter_bmdma_prepare(
	struct PATAAdapter *dev, int channel, phyaddr_t addr, unsigned int size
) {
