...
static inline void
ahci_write_reg32(struct AHCIController *ahci, unsigned int offset, uint32_t value) {
...
void ahci_init_port(struct AHCIController *ahci, unsigned int port) {
	...
	if (((ahci_read_reg32(ahci, AHCI_PORT_CONTROL_OFFSET(port) + AHCI_PxSSTS) >>
		 ...
		 AHCI_PxSSTS_DET_MASK) == AHCI_PxSSTS_DET_PRESENSE_COMM) {
		if (((ahci_read_reg32(ahci, AHCI_PORT_CONTROL_OFFSET(port) + AHCI_PxSSTS) >>
	...
	ahci_write_reg32(
		ahci, AHCI_PORT_CONTROL_OFFSET(port) + AHCI_PxFB_LOWER, V2P(received_fis_buffer)
	);
	ahci_write_reg32(ahci, AHCI_PORT_CONTROL_OFFSET(port) + AHCI_PxFB_UPPER, 0);
	// enable receive FIS
	ahci_write_reg32(
		ahci,
		AHCI_PORT_CONTROL_OFFSET(port) + AHCI_PxCMD,
		ahci_read_reg32(ahci, AHCI_PORT_CONTROL_OFFSET(port) + AHCI_PxCMD) | AHCI_PxCMD_FRE
	);
...
void ahci_controller_init(struct PCIDevice *pci_dev) {
	...
	cprintf(
		...
		BOOL2SIGN(ahcicap & AHCI_CAP_SAM),
...
int ahci_exec_pio_in(
	struct AHCIController *ahci, unsigned int port, uint8_t cmd, unsigned long long lba,
	...
	ahci_write_reg32(
		ahci,
		AHCI_PORT_CONTROL_OFFSET(port) + AHCI_PxCMD,
		ahci_read_reg32(ahci, AHCI_PORT_CONTROL_OFFSET(port) + AHCI_PxCMD) | AHCI_PxCMD_ST
	);
	ahci_write_reg32(ahci, AHCI_PORT_CONTROL_OFFSET(port) + AHCI_PxIS, 0xffffffff);
	// issue command
	while ((ahci_read_reg32(ahci, AHCI_PORT_CONTROL_OFFSET(port) + AHCI_PxTFD) &
			AHCI_PxTFD_STATUS_BSY) ||
